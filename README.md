...
## Лабораторна робота №9 — масиви та об’єкти в JavaScript

Дані карток перенесено з HTML у масив `products` у файлі `assets/js/main.js`. До чотирьох наявних карток додано ще чотири. Кожен об’єкт містить `id`, назву, ціну, категорію та посилання на зображення.

Картки створюються через `map()` і шаблонні рядки, об’єднуються за допомогою `join("")` та виводяться у `.products-grid` через `innerHTML`. У `pages/index.html` залишено порожню сітку та підключено скрипт з `type="module"`. Стилі й оформлення попередніх карток збережено.

### Перевірка

Відкрити [сайт](https://nikitauroboros-cpu.github.io/SilkovNikitaKNK25/) і перейти до розділу «Мої навички». На сторінці мають відображатися 8 карток. Для локальної перевірки потрібно запускати сайт через локальний сервер, наприклад Live Server.
