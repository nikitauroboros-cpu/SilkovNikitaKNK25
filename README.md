...
## Лабораторна робота №9 — картки навичок з JavaScript

У файлі `assets/js/main.js` створено масив `skills` з вісьмома навичками. Кожна має id, назву, категорію, опис, рівень та зображення. Цін немає, оскільки це портфоліо.

Картки виводяться через `map()`, шаблонні рядки, `join("")` та `innerHTML`. У `pages/index.html` залишено порожній контейнер `.products-grid` і підключено скрипт як модуль. Оформлення карток збережено.

## Лабораторна робота №10 — обрані навички

Логіку кошика адаптовано до портфоліо: відвідувач може скласти список навичок, які його зацікавили.

- Один обробник кліків на сітці карток використовує делегування подій.
- Навичка знаходиться за id через `find()` і додається до масиву `selectedSkills` за допомогою spread-оператора.
- Повторний клік прибирає навичку зі списку, дублікати не створюються.
- `reduce()` рахує кількість обраних навичок замість вартості покупок.
- Лічильник у шапці та список обраного оновлюються після кожної зміни. Є кнопка очищення.
- Стан зберігається в пам’яті сторінки та скидається після оновлення.

Це адаптація завдання: грошову суму та кількість одиниць одного товару замінено підрахунком унікальних обраних навичок.

### Перевірка

1. Відкрити [сайт](https://nikitauroboros-cpu.github.io/SilkovNikitaKNK25/). У розділі «Мої навички» мають бути 8 карток без цін.
2. Обрати дві навички — лічильник показує 2, а в «Обрані навички» видно їхні назви.
3. Натиснути повторно на одну з них — вона зникає з обраного, лічильник показує 1.
4. Натиснути «Очистити обране» — список порожній, лічильник показує 0.
5. У консолі браузера можна переглянути масив обраного та його кількість.

Локально сайт потрібно відкривати через HTTP-сервер, наприклад Live Server, щоб працював модуль JavaScript.
